...
```
totalCpu =  
 ztunnelCpuTotal + waypointCpuTotal + istiodCpuTotal  
```
---
...
```
totalMemoryMB =  
 ztunnelMemTotalMB + waypointMemTotalMB + istiodMemTotalMB  
```
---
...
| waypoint Memory | Traffic | ~300 MB + variable |
...
## 10. CNI Sizing Recommendation

Final Recommendation (Based on General Consensus and Benchmarks)

CPU overhead for CNI:
Negligible. Typically, the CNI doesn't consume much CPU directly unless network policies are complex or there’s a significant amount of pod-to-pod traffic with encryption.

Memory overhead for CNI:
